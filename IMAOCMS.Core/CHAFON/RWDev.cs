...
namespace IMAOCMS.Core.CHAFON
{
    ...
    public static class RWDev
    {
        ...
        [DllImport(DLLNAME, CallingConvention = CallingConvention.StdCall)]
        public static extern int GetReaderInformation(ref byte ComAdr,              //okuyucu adresi		
                                                      byte[] VersionInfo,           //Yazılım versiyonu
                                                      ref byte ReaderType,              //Okuyucu modeli
                                                      ref byte TrType,      //desteklenen protokoller
                                                      ref byte dmaxfre,           //Şu anda okuyucu tarafından kullanılan en yüksek frekans
                                                      ref byte dminfre,           //Şu anda okuyucu tarafından kullanılan en düşük frekans
                                                      ref byte powerdBm,             //Okuyucu çıkış gücü
                                                      ref byte ScanTime,
                                                      ref byte Ant,
                                                      ref byte BeepEn,
                                                      ref byte OutputRep,
            ...
            while (!toStopThread)
            {
                ...
                if (fCmdRet == 0)
                {
                    ...
                    try
                    {
                        ...
                        fInventory_EPC_List = fInventory_EPC_List + temp;//dizeleri listede sakla
                        nLen = fInventory_EPC_List.Length;
                        while (fInventory_EPC_List.Length > 18)
                        {
                            string FlagStr = "EE00";//Başlık konumu bayrak dizesini bulun
                            ...
                            NumLen = Convert.ToInt32(fInventory_EPC_List.Substring(0, 2), 16) * 2 + 2;//İlk çerçevenin uzunluğunu alın
